...
class community_moderation
{
	...
	*/
	function record_vote_info(&$vmode, &$post_id, $auth, $viewtopic_url, $forum_id)
	{
		...
		if ($vmode == 'upvote')
		{
			if ($this->ultimate_points_installed() && $this->config['enable_ups'])
			{
				$this->do_ultimate_points('upvote', $row['poster_id']);
			}
		...
		else
		{
			if ($this->ultimate_points_installed() && $this->config['enable_ups'])
			{
				$this->do_ultimate_points('downvote', $row['poster_id']);
			}
	...
	*/
	...
	/**
	* function do_ultimate_points
	*/
	function do_ultimate_points($vmode, $user_id)
	{
		if ($vmode == 'upvote')
		{
			add_points($user_id, $this->config['ups_upvote']);
		}
		else
		{
			if ($this->config['ups_downvote'] <> 0)
			{
				substract_points($user_id, $this->config['ups_downvote']);
			}
		}
	}
